...
cv_quad_v2_var_improve_all = []

for i in range(100):
    # for different random seed
    time.sleep(1.0)
    posterior = stan.build(model_str, data=data, random_seed=int(datetime.timestamp(datetime.now())))
    fit = posterior.sample(num_chains=2, num_samples=1000, num_warmup=1000)
    ...
    cv_quad_v2_mean = np.mean(cv_samples_quadratic_v2, axis=0)
    ...
    cv_quad_v2_rmse = np.sqrt(np.mean((cv_quad_v2_mean - larger_samples_mean)**2))
    print('means rmse: {:f} -- {:f} -- {:f} -- {:f}'.format(raw_rmse, cv_linear_rmse, cv_quad_rmse, cv_quad_v2_rmse))
    raw_rmse_all.append(raw_rmse)
    cv_linear_rmse_all.append(cv_linear_rmse)
    cv_quad_rmse_all.append(cv_quad_rmse)
    cv_quad_v2_rmse_all.append(cv_quad_v2_rmse)
    ...
    cv_quad_v2_var = np.var(cv_samples_quadratic_v2, axis=0)
    print('control variates (quadratic v2) sample variances:')
    print(cv_quad_v2_var)
    cv_quad_v2_var_improve_all.append(np.mean(raw_samples_var/cv_quad_v2_var))
...
report = pd.DataFrame({'Sample RMSE':raw_rmse_all, 'Linear CV RMSE': cv_linear_rmse_all,
'Quad CV RMSE': cv_quad_rmse_all, 'Quad CV extended RMSE': cv_quad_v2_rmse_all,
'Linear CV Var Reduction': cv_linear_var_improve_all, 
'Quad CV Var Reduction': cv_quad_var_improve_all, 'Quad CV extended Var Reduction':cv_quad_v2_var_improve_all})
report.to_csv(model_name + '_report.csv', index=False)
